...
class FeatMatchTrainer(ssltrainer.SSLTrainer):
    ...
    def init_model(self):
        model = FeatMatch(backbone=self.config['model']['backbone'], # wide-residual-network
                          num_classes=self.config['model']['classes'], # 10
                          devices=self.args.devices,
                          num_heads=self.config['model']['num_heads'], # 4
                          amp=self.args.amp)
    ...
    def train1(self, xl, yl, xu):
        ...
        bsl, bsu, c = len(xl), len(xu), self.config['model']['classes'] # batch size labeled: 64, batch size unlabeled:128, k:None, 10
        x = torch.cat([xl,xu], dim=0) # (192, 3, 32, 32)
        logits_x = self.model(x) # (192, 10)
        logits_xl, logits_xu = logits_x[:bsl], logits_x[bsl:] # (64, 10), (128, 10)

        # Compute pseudo label
        prob_xu_fake = torch.softmax(logits_xu.detach(), dim=1) #(128, 10)
        ...
        #prob_xu_fake = prob_xu_fake.unsqueeze(1).repeat(1, k, 1) # k번 repeat (128, 9, 10)

        # Mixup perturbation
        #xu = xu.reshape(-1, *xu.shape[2:]) # (128*9, 3, 32, 32)
        #xl = xl.reshape(-1, *xl.shape[2:]) # (64*9, 3, 32, 32)

        prob_xl_gt = torch.zeros(len(xl), c, device=xl.device) # (64, 10) 왜 0임?
        #prob_xl_gt.scatter_(dim=1, index=yl.unsqueeze(1).repeat(1, k).reshape(-1, 1), value=1.) # (64*9, 10)
        prob_xl_gt.scatter_(dim=1, index=yl.unsqueeze(1), value=1.)

        #####################################

        xl_mix, probl_mix, xu_mix, probu_mix = self.data_mixup(xl, prob_xl_gt, xu, prob_xu_fake.reshape(-1, c)) # (64*9, 3, 32, 32), (64*9, 10), (128*9, 3, 32, 32), (128*9, 10)
        
        # Forward pass on mixed data
        Nl = len(xl_mix) # (64*9, 3, 32, 32)
        x_mix = torch.cat([xl_mix, xu_mix], dim=0) #(192*9, 3, 32, 32)
        logits_x_mix = self.model(x_mix) # (192*9, 10)
        logits_xl_mix, logits_xu_mix = logits_x_mix[:Nl], logits_x_mix[Nl:] # (64*9, 10), (128*9, 10)
        ...
        # Prediction
        pred_x = torch.softmax(logits_xl.detach(), dim=1) # (64, 10)
        # import pdb
        # pdb.set_trace()
        return pred_x, loss, loss_pred, loss_con, loss_graph

    def train2(self, xl, yl, xu):
        bsl, bsu, k, c = len(xl), len(xu), xl.size(1), self.config['model']['classes'] # 64, 128, none, 10
        #x = torch.cat([xl, xu], dim=0).reshape(-1, *xl.shape[2:])
        x = torch.cat([xl,xu], dim=0)
        logits_xg, _, fx, _, _ = self.model(x, self.fp) #(192, 10), _, (192, 128), _, _
        #logits_xg = logits_xg.reshape(bsl + bsu, k, c)
        logits_xgl, logits_xgu = logits_xg[:bsl], logits_xg[bsl:] #(64,10), (128,10)
        #fxu = fx.reshape(bsl + bsu, k, fx.size(1))[bsl:] # (128, k, 128)
        fxu = fx #(128, 128)
        #self.fu.append(fxu[:, 0].detach().clone().float().cpu())
        self.fu.append(fxu.detach().clone().float().cpu())

        # Compute pseudo label
        #prob_xu_fake = torch.softmax(logits_xgu[:, 0].detach(), dim=1)
        prob_xu_fake = torch.softmax(logits_xgu.detach(), dim=1) # detach 해줘야 하는지 모르곘음
        ...
        # Prediction
        pred_x = torch.softmax(logits_xgl.detach(), dim=1)
    ...
    def forward_train(self, data):
        # data[0] : xl, data[1] : yl, data[2] : xu , (64, 3, 32, 32), (64), (128, 3, 32, 32)
        self.model.train()
        #xl = data[0].reshape(-1, *data[0].shape[2:]) # (64*9, 3, 32, 32)
        xl = data[0]
        xl = self.Tnorm(xl.to(self.default_device)) # ZCA Norm : raw data를 zca를 해?, (64, 3, 32, 32)
        yl = data[1].to(self.default_device) # (64,)
        #xu = data[2].reshape(-1, *data[2].shape[2:]) # (128, 3, 32, 32)
        xu = data[2]
        xu = self.Tnorm(xu.to(self.default_device)) # (128, 3, 32, 32)

        if self.curr_iter < self.config['train']['pretrain_iters']: # less than 3000
            self.model.set_mode('pretrain')
            pred_x, loss, loss_pred, loss_con, loss_graph = self.train1(xl, yl, xu)
        elif self.curr_iter == self.config['train']['pretrain_iters']: # 3000
            self.model.set_mode('train')
            self.extract_fp()
            pred_x, loss, loss_pred, loss_con, loss_graph = self.train2(xl, yl, xu)
        else:
            self.model.set_mode('train')
            if self.curr_iter % self.config['train']['sample_interval'] == 0: # 3000 이후로 500 주기로 
                self.extract_fp()
            pred_x, loss, loss_pred, loss_con, loss_graph = self.train2(xl, yl, xu)
